...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!isset($data['anointingID']) || empty($data['anointingID']) || 
            ...
            handleError("Missing required fields (anointingID and status)");
        }
        ...
        // Update the anointing status
        $stmt = $conn->prepare("UPDATE anointing_application SET status = ? WHERE anointingID = ?");
        if (!$stmt) {
            handleError("Prepare statement failed", $conn->error);
        }
        
        $stmt->bind_param("si", $status, $anointingID);
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->affected_rows > 0) {
                ...
                $response["message"] = "Anointing status updated successfully";
                
                // If status is "Approved", send email notification
                if ($status === "Approved") {
                    // Make a request to the approved_anointing_email.php script
                    $emailData = json_encode(["anointingID" => $anointingID]);
                    
                    $ch = curl_init();
                    
                    $url = 'https://parishofdivinemercy.com/backend/approved_anointing_email.php';
                ...
                // No rows were affected - anointingID probably doesn't exist
                handleError("No anointing record found with ID: " . $anointingID);
            }
        } else {
            // Failed to update status
            handleError("Failed to update anointing status", $conn->error);
        }
