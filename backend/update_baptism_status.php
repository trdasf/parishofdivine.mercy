...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!isset($data['baptismID']) || empty($data['baptismID']) || 
            ...
            handleError("Missing required fields (baptismID and status)");
        }
        ...
        // Update the baptism status
        $stmt = $conn->prepare("UPDATE baptism_application SET status = ? WHERE baptismID = ?");
        if (!$stmt) {
            handleError("Prepare statement failed", $conn->error);
        }
        
        $stmt->bind_param("si", $status, $baptismID);
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->affected_rows > 0) {
                ...
                $response["message"] = "Baptism status updated successfully";
                
                // If status is "Approved", send email notification
                if ($status === "Approved") {
                    // Make a request to the approved_baptism_email.php script
                    $emailData = json_encode(["baptismID" => $baptismID]);
                    
                    $ch = curl_init();
                    
                    $url = 'https://parishofdivinemercy.com/backend/approved_baptism_email.php';
                ...
                // No rows were affected - baptismID probably doesn't exist
                handleError("No baptism record found with ID: " . $baptismID);
            }
        } else {
            // Failed to update status
            handleError("Failed to update baptism status", $conn->error);
        }
