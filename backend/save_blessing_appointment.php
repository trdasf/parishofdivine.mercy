...
ini_set('error_log', '/path/to/error_log.log'); // Make sure this path is writable
...
function handleError($message, $errorDetails = null) {
    ...
    // Log the error
    error_log("ERROR: $message " . ($errorDetails ? "Details: $errorDetails" : ""));
...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        // Log the values that will be inserted/updated
        error_log("Values to save: ID=$sacramentID, Type=$sacrament_type, Date=$date, Time=$time, Priest=$priest");
        ...
        if ($checkResult->num_rows > 0) {
            ...
            if ($updateStmt->execute()) {
                ...
                error_log("Appointment updated successfully for ID=$appointmentID");
            } else {
            ...
            if ($insertStmt->execute()) {
                ...
                error_log("New appointment inserted with ID=" . $conn->insert_id);
            } else {
