...
try {
    ...
    if (!$data || !isset($data->firstName, $data->lastName, $data->fatherFirstName, $data->fatherLastName, 
                         $data->motherFirstName, $data->motherLastName, $data->placeOfBaptism, $data->dateOfBaptism)) {
    ...
    $place_of_baptism = $conn->real_escape_string(trim($data->placeOfBaptism));
    $date_of_baptism = $conn->real_escape_string(trim($data->dateOfBaptism));
    ...
    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $date_of_baptism)) {
        throw new Exception("Invalid date format for baptism date. Please use YYYY-MM-DD format.");
    }
    ...
    if (isset($data->purposeCommConfir) && $data->purposeCommConfir) {
        $purposes[] = "First Communion / Confirmation";
    }
    if (isset($data->purposeSchool) && $data->purposeSchool) {
        $purposes[] = "School Requirements";
    }
    ...
    if (empty($purposes)) {
        throw new Exception("Please select at least one purpose for the baptism certificate request.");
    }

    // Convert purposes array to string
    $purpose_string = implode(", ", $purposes);

    // Insert the baptism certificate request
    $stmt = $conn->prepare("INSERT INTO request_baptism (first_name, middle_name, last_name, father_fname, father_mname, father_lname, mother_fname, mother_mname, mother_lname, place_of_baptism, date_of_baptism, name_of_priest, purpose, clientID) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    $stmt->bind_param("sssssssssssssi", 
        ...
        $place_of_baptism, $date_of_baptism, $name_of_priest,
        $purpose_string, $clientID
    );
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode([
            "success" => true, 
            "message" => "Baptism certificate request submitted successfully.",
            "requestID" => $request_id,
            "data" => [
                "reqbaptismID" => $request_id,
    ...
    error_log("Baptism certificate request error: " . $e->getMessage());
