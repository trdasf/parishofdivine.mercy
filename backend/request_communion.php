...
try {
    ...
    if (!$data || !isset($data->firstName, $data->lastName, $data->fatherFirstName, $data->fatherLastName, 
                         $data->motherFirstName, $data->motherLastName, $data->placeOfCommunion, $data->dateOfCommunion)) {
    ...
    $place_of_communion = $conn->real_escape_string(trim($data->placeOfCommunion));
    $date_of_communion = $conn->real_escape_string(trim($data->dateOfCommunion));
    ...
    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $date_of_communion)) {
        throw new Exception("Invalid date format for communion date. Please use YYYY-MM-DD format.");
    }
    ...
    if (isset($data->purposeConfirmation) && $data->purposeConfirmation) {
        $purposes[] = "Confirmation Preparation";
    }
    ...
    if (empty($purposes)) {
        throw new Exception("Please select at least one purpose for the communion certificate request.");
    }

    // Convert purposes array to string
    $purpose_string = implode(", ", $purposes);

    // Insert the communion certificate request
    $stmt = $conn->prepare("INSERT INTO request_communion (clientID, first_name, middle_name, last_name, father_fname, father_mname, father_lname, mother_fname, mother_mname, mother_lname, place_of_communion, date_of_communion, name_of_priest, purpose) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    $stmt->bind_param("isssssssssssss", 
        ...
        $place_of_communion, $date_of_communion, $name_of_priest,
        $purpose_string
    );
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode([
            "success" => true, 
            "message" => "First Holy Communion certificate request submitted successfully.",
            "requestID" => $request_id,
            "data" => [
                "reqcommunionID" => $request_id,
    ...
    error_log("Communion certificate request error: " . $e->getMessage());
