...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!isset($data['marriageID']) || empty($data['marriageID']) || 
            ...
            handleError("Missing required fields (marriageID and status)");
        }
        ...
        // Update the marriage status
        $stmt = $conn->prepare("UPDATE marriage_application SET status = ? WHERE marriageID = ?");
        if (!$stmt) {
            handleError("Prepare statement failed", $conn->error);
        }
        
        $stmt->bind_param("si", $status, $marriageID);
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->affected_rows > 0) {
                ...
                $response["message"] = "Marriage status updated successfully";
                
                // If status is "Approved", send email notification
                if ($status === "Approved") {
                    // Make a request to the approved_marriage_email.php script
                    $emailData = json_encode(["marriageID" => $marriageID]);
                    
                    $ch = curl_init();
                    
                    $url = 'https://parishofdivinemercy.com/backend/approved_marriage_email.php';
                ...
                // No rows were affected - marriageID probably doesn't exist
                handleError("No marriage record found with ID: " . $marriageID);
            }
        } else {
            // Failed to update status
            handleError("Failed to update marriage status", $conn->error);
        }
