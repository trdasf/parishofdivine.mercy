...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!isset($data['blessingID']) || empty($data['blessingID']) || 
            ...
            handleError("Missing required fields (blessingID and status)");
        }
        ...
        // Update the blessing status
        $stmt = $conn->prepare("UPDATE blessing_application SET status = ? WHERE blessingID = ?");
        if (!$stmt) {
            handleError("Prepare statement failed", $conn->error);
        }
        
        $stmt->bind_param("si", $status, $blessingID);
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->affected_rows > 0) {
                ...
                $response["message"] = "Blessing status updated successfully";
                
                // If status is "Approved", send email notification
                if ($status === "Approved") {
                    // Make a request to the approved_blessing_email.php script
                    $emailData = json_encode(["blessingID" => $blessingID]);
                    
                    $ch = curl_init();
                    
                    $url = 'https://parishofdivinemercy.com/backend/approved_blessing_email.php';
                ...
                // No rows were affected - blessingID probably doesn't exist
                handleError("No blessing record found with ID: " . $blessingID);
            }
        } else {
            // Failed to update status
            handleError("Failed to update blessing status", $conn->error);
        }
