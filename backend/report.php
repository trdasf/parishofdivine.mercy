...
try {
    ...
    elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!$data || !isset($data->expenseName, $data->category, $data->amount, $data->quantity)) {
            throw new Exception("Missing required fields for expense report.");
        }
        
        // Calculate total cost
        $totalCost = $data->amount * $data->quantity;
    ...
    elseif ($_SERVER['REQUEST_METHOD'] === 'PUT') {
        ...
        // Validate received data
        if (!$data || !isset($data->reportID, $data->expenseName, $data->category, $data->amount, $data->quantity)) {
