...
import "./BlessingView.css";
import jsPDF from 'jspdf';
import html2canvas from 'html2canvas';

const BlessingView = () => {
  const location = useLocation();
  // Access blessingData correctly from location state
  ...
  // For debugging
  useEffect(() => {
    console.log("Received blessing data:", blessingData);
  }, [blessingData]);
  ...
  const renderReadOnlyField = (value) => {
    return <div className="blessing-view-value">{value || "N/A"}</div>;
  };

  // Function to render document status with view button
  const renderDocumentStatus = (isSubmitted, fileName) => {
    return (
      <div className="blessing-document-status-container">
        <div className={`blessing-view-status ${isSubmitted ? 'blessing-view-submitted' : 'blessing-view-not-submitted'}`}>
          {isSubmitted ? `Submitted: ${fileName}` : "Not Submitted"}
        </div>
        {isSubmitted && fileName && (
          <button 
            className="blessing-view-document-btn"
  ...
  const renderDocumentViewer = () => {
    ...
    return (
      <div className="blessing-document-viewer-overlay">
        <div className="blessing-document-viewer-container">
          <div className="blessing-document-viewer-header">
            <h3>{viewingDocument}</h3>
            <button 
              className="blessing-document-close-btn"
              onClick={() => setViewingDocument(null)}
            >
              ×
            </button>
          </div>
          <div className="blessing-document-viewer-content">
            {/* In a real application, this would display the actual document */}
            <div className="blessing-document-placeholder">
  ...
  const renderRequirements = () => {
    ...
    // Initialize requirements with empty object if not present
    const requirements = blessingData.requirements || {};
    
    // Always show valid ID requirement
    requirementsList.push(
      <div key="validId" className="blessing-requirement-view-item">
        <div className="blessing-view-requirement-name">
          Valid ID of the Requester
        </div>
        {renderDocumentStatus(
          requirements.valid_id?.submitted || false, 
    ...
    switch(blessingData.blessingType) {
      case "house":
        requirementsList.push(
          <div key="proofOfOwnership" className="blessing-requirement-view-item">
            <div className="blessing-view-requirement-name">
              Proof of Ownership
            </div>
            {renderDocumentStatus(
              requirements.proof_of_ownership?.submitted || false, 
              requirements.proof_of_ownership?.fileName || ""
            )}
          </div>
        );
        requirementsList.push(
          <div key="barangayClearance" className="blessing-requirement-view-item">
            <div className="blessing-view-requirement-name">
              Barangay Clearance
            </div>
            {renderDocumentStatus(
              requirements.barangay_clearance?.submitted || false, 
      ...
      case "business":
        requirementsList.push(
          <div key="businessPermit" className="blessing-requirement-view-item">
            <div className="blessing-view-requirement-name">
              Business Permit / DTI Registration
            </div>
            {renderDocumentStatus(
              requirements.business_permit?.submitted || false, 
      ...
      case "car":
        requirementsList.push(
          <div key="vehicleRegistration" className="blessing-requirement-view-item">
            <div className="blessing-view-requirement-name">
              Vehicle OR/CR (Official Receipt / Certificate of Registration)
            </div>
            {renderDocumentStatus(
              requirements.vehicle_registration?.submitted || false, 
  ...
  const renderBlessingInfo = () => {
    switch(blessingData.blessingType) {
      case "house":
        return (
          <div className="blessing-info-view-list">
            <div className="blessing-info-view-item">
  ...
  const renderCertificateTemplate = () => {
    ...
    switch(blessingData.blessingType) {
      ...
      default:
        return (
          <div className="blessing-certificate-content">
            <p>Invalid blessing type: {blessingData.blessingType}</p>
          </div>
        );
    }
  };
  ...
  const renderCertificateModal = () => {
    ...
    return (
      <div className="blessing-document-viewer-overlay">
        <div className="blessing-certificate-modal-container">
          <div className="blessing-document-viewer-header">
          ...
          <div className="blessing-certificate-modal-content">
            ...
            <div className="blessing-certificate-modal-actions">
              <button 
                className="blessing-certificate-download-btn"
              ...
              <button 
                className="blessing-certificate-cancel-btn"
                onClick={() => setShowCertificateModal(false)}
                disabled={isDownloading}
              >
                Cancel
  ...
  if (!blessingData || Object.keys(blessingData).length === 0) {
    return (
      <div className="blessing-view-container">
        <div className="blessing-view-header">
          <div className="blessing-view-left-section">
            <button className="blessing-view-back-button" onClick={() => window.history.back()}>
              <AiOutlineArrowLeft className="blessing-view-back-icon" /> Back
            </button>
          </div>
        </div>
        <div className="blessing-view-error">
  ...
  return (
    <div className="blessing-view-container">
      ...
      <div className="blessing-view-header">
        ...
        <div className="blessing-view-right-section">
          <div className="blessing-view-status-display">
            Status: <span className={`blessing-status-badge ${status.toLowerCase()}`}>{status}</span>
          </div>
          <button 
            className="blessing-download-certificate-btn"
      ...
      <h1 className="blessing-view-title">Blessing Ceremony Details</h1>
      
      {/* Blessing Data Section */}
      <div className="blessing-view-data">
        <div className="blessing-view-info-card">
          <div className="blessing-view-row-date">
            <div className="blessing-view-field-date">
            ...
            <div className="blessing-view-field-time">
        ...
        <div className="blessing-view-bypart">
          <h2 className="blessing-view-sub-title">Personal Information</h2>
          
          <div className="blessing-view-info-card">
            <div className="blessing-view-row">
              <div className="blessing-view-field">
            ...
            <div className="blessing-view-row blessing-address-view-row">
            <div className="blessing-view-field">
          ...
          <h2 className="blessing-view-sub-title">Blessing Details</h2>
        ...
        <div className="blessing-requirements-view-container">
          <h2 className="blessing-requirements-view-title">Requirements</h2>
          <div className="blessing-requirements-view-box">
            <h3 className="blessing-view-section-header">Documents Status</h3>
            <div className="blessing-view-checkbox-list">
              {renderRequirements()}
            </div>

            <h3 className="blessing-view-section-header">
